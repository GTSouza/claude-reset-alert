...
| `dashboard` | Gera um **dashboard HTML autocontido** (CSS+SVG+JS inline, custo zero — só lê o SQLite): status Claude+Codex com veredito do gate, custo/dia (14d) **empilhado por fonte** (assinatura × crédito), custo por modelo em **abas hoje/7d/30d** (in/out/%out/cache_r/cache_w), **top sessões** e **por projeto** (7d), sparkline do medidor 5h (24h), billing, **episódios de crédito + renovações do plano**, sessões Codex por dia e eventos recentes. **Tooltips ricos** (HTML): o ~USD por modelo abre a decomposição tokens×preço por componente (com o fator calibrado), e cada sessão mostra título do transcript, workspace, período, modelos e id. `--open` abre no navegador; `--out`/`DASHBOARD_PATH` mudam o destino (default `~/.claude/tools/dashboard.html`). |
