...
| `codex-report` | Uso de tokens do **Codex** por sessão/dia/modelo (tokens + tempo ativo; assinatura, sem custo/token). `--by model` usa os deltas por turno (`codex_model_usage`) — troca de modelo no meio da sessão conta certo — ver seção própria abaixo. |
...
| `dashboard` | Gera um **dashboard HTML autocontido** (CSS+SVG+JS inline, custo zero — só lê o SQLite): status Claude+Codex com veredito do gate e idade da leitura; custo/dia (14d) **empilhado por fonte** com tooltip de quebra por modelo; custo por modelo em **abas hoje/7d/30d** (in/out/%out/cache_r/cache_w, linha TOTAL); **top sessões** (com projeto e **tempo ativo**) e **por projeto** (com nº de sessões e tempo ativo somado); **três sparklines** (Claude 5h 24h, Claude semanal 7d, Codex 5h 24h); **billing hoje+7d** (msgs/out/~USD por fonte); **episódios de crédito** (com custo estimado por modelo no tooltip) + renovações; **preços & calibração** (a base do ~USD); Codex **por modelo**, **top sessões** e **por dia** (in/cached/out/total/tempo); eventos recentes com os percentuais no instante. **Tooltips ricos** (HTML): todo ~USD abre a decomposição por modelo/componente (com o fator calibrado) — nas abas por modelo, nos rankings e no billing; sessões Claude mostram título do transcript, workspace, período, modelos, tokens, crédito e branch; sessões Codex mostram título, workspace (`cwd`), período, modelos e tokens (o total também abre a quebra por modelo). `--open` abre no navegador; `--out`/`DASHBOARD_PATH` mudam o destino (default `~/.claude/tools/dashboard.html`). |
...
O `ingest` também varre os rollouts e grava o uso de tokens por sessão na tabela `codex_usage` (total cumulativo do último `token_count` + janela de tempo 1º→último evento + modelo + **workspace** do `session_meta.cwd` + **título**, a 1ª mensagem de texto do usuário). Além disso, os **deltas** de cada `token_count` são atribuídos ao modelo do `turn_context` vigente e agregados em `codex_model_usage` — o `--by model` separa por modelo **mesmo quando a sessão troca de modelo no meio** (e um contador que reinicia no rollout é tratado). Bancos antigos são migrados e re-ingeridos automaticamente no próximo `ingest`. Codex é assinatura, então reporta **tokens e tempo**, não custo por token.
