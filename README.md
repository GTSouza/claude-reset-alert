...
| `dashboard` | Gera um **dashboard HTML autocontido** (CSS+SVG+JS inline, custo zero — só lê o SQLite) com 15 seções: status Claude+Codex com veredito do gate e idade da leitura; custo/dia (14d) **empilhado por fonte** com tooltip de quebra por modelo; custo por modelo em **abas hoje/7d/30d** (in/out/%out/cache_r/cache_w, linha TOTAL); **top sessões** (com projeto) e **por projeto** (com nº de sessões); **três sparklines** (Claude 5h 24h, Claude semanal 7d, Codex 5h 24h); **billing hoje+7d** (msgs/out/~USD por fonte); **episódios de crédito** (com custo estimado por modelo no tooltip) + renovações; **batidas de limite** (14d); **preços & calibração** (a base do ~USD); Codex **por modelo**, **top sessões** e **por dia** (in/cached/out/total/tempo); eventos recentes com os percentuais no instante. **Tooltips ricos** (HTML): o ~USD por modelo abre a decomposição tokens×preço por componente (com o fator calibrado); sessões Claude mostram título do transcript, workspace, período, modelos, tokens, crédito e branch; sessões Codex mostram título, workspace (`cwd`), período, modelos e tokens. `--open` abre no navegador; `--out`/`DASHBOARD_PATH` mudam o destino (default `~/.claude/tools/dashboard.html`). |
